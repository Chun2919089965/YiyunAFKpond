...
dependencies {
    compileOnly("com.github.Chun2919089965:YiyunAFKpond:1.0.3")
}
```
...
```bash
mvn install:install-file -Dfile=YiyunAFKpond-1.0.3.jar -DgroupId=com.yiyunafkpond -DartifactId=YiyunAFKpond -Dversion=1.0.3 -Dpackaging=jar
```
...
> **提示**：使用 `provided` scope 表示编译时依赖，运行时由服务端插件提供。
