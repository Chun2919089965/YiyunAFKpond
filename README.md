...
| 家具/载具兼容 | 易漏判玩家状态    | ✅ **骑乘事件 + 周期自愈**，兼容 CraftEngine 家具座椅         |
...
- **传送拦截** —— 阻止玩家从池外传送进入挂机池区域，同池内安全下座不受影响（可绕过权限）
- **权限隔离** —— 无权限玩家无法进入受限池
- **坐席安全** —— 通过家具或载具进入受限池时同样执行权限与同 IP 限制
...
core:
  debug: false
  auto-complete: true
  presence-reconcile-interval: 1 # 家具、载具和外部插件移动的位置自愈间隔（秒）
