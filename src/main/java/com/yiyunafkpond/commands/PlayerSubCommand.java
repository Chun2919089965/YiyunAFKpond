...
public class PlayerSubCommand implements SubCommand {
    ...
    private void sendHelp(CommandSender sender) {
        ...
        plugin.sendPlayerMessage(sender, "&#B0E0E6/yafk player info <玩家> &#ADD8E6- 查看玩家信息");
        plugin.sendPlayerMessage(sender, "&#B0E0E6/yafk player reset <玩家> [daily|pond <池ID>] &#ADD8E6- 重置玩家数据");
        plugin.sendPlayerMessage(sender, "&#B0E0E6/yafk player set <玩家> <属性> <值> &#ADD8E6- 设置玩家数据");
        plugin.sendPlayerMessage(sender, "&#B0E0E6/yafk player stats <玩家> &#ADD8E6- 查看玩家统计");
        plugin.sendPlayerMessage(sender, "&#B0E0E6/yafk player list [afk] &#ADD8E6- 列出在线玩家");
    }
    
    private boolean handleInfo(CommandSender sender, String[] args) {
        if (args.length < 3) { plugin.sendPlayerMessage(sender, "&#6CA6CD用法：/yafk player info <玩家>"); return true; }
    ...
    private boolean handleReset(CommandSender sender, String[] args) {
        if (args.length < 3) { plugin.sendPlayerMessage(sender, "&#6CA6CD用法：/yafk player reset <玩家> [daily|pond <池ID>]"); return true; }
        Player target = Bukkit.getPlayerExact(args[2]);
        if (target == null) { plugin.sendPlayerMessage(sender, "&#6CA6CD玩家不存在或未在线"); return true; }
        PlayerData data = plugin.getDataManager().getPlayerData(target.getUniqueId());
        ...
        if (args.length < 4) {
            ...
            plugin.sendPlayerMessage(sender, "&#6CA6CD用法：/yafk player reset <玩家> [daily|pond <池ID>]");
        }
        return true;
    }
    ...
    private boolean handleSet(CommandSender sender, String[] args) {
        if (args.length < 5) { 
            plugin.sendPlayerMessage(sender, "&#6CA6CD用法：/yafk player set <玩家> <属性> <值>");
        ...
        switch (property) {
            ...
            case "pond" -> {
                if (args.length < 7) { plugin.sendPlayerMessage(sender, "&#6CA6CD用法：/yafk player set <玩家> pond <池ID> <exp|money|point> <值>"); return true; }
                String pondId = args[4];
    ...
    private boolean handleStats(CommandSender sender, String[] args) {
        if (args.length < 3) { plugin.sendPlayerMessage(sender, "&#6CA6CD用法：/yafk player stats <玩家>"); return true; }
        Player target = Bukkit.getPlayerExact(args[2]);
        if (target == null) { plugin.sendPlayerMessage(sender, "&#6CA6CD玩家不存在或未在线"); return true; }
        PlayerData data = plugin.getDataManager().getPlayerData(target.getUniqueId());
